  ...
  <DropdownMenu.Root>
    ...
    <DropdownMenu.Content class="z-[99999]  min-w-64 overflow-hidden rounded-lg bg-background-grey/95 text-3xl font-semibold" align="start" side="bottom" transition={flyAndScale} transitionConfig={{ y: -8, duration: 150 }}>
      {#each profile.profiles as otherProfile}
        <DropdownMenu.Item href={`/stats/${profile.username}/${otherProfile.cute_name}`} class="flex items-center px-4 py-2 hover:bg-text/20" data-sveltekit-preload-code="viewport">
...
<div class="flex flex-wrap items-center gap-x-4 gap-y-2">
  ...
  <Button.Root href={`https://plancke.io/hypixel/player/stats/${profile.username}`} class="flex items-center justify-center gap-1.5 rounded-full bg-icon/90 px-2 py-1 font-semibold transition-opacity duration-150 hover:bg-icon">
