...
const files = fs.readdirSync('static/resourcepacks/Vanilla/assets/minecraft/mcpatcher/cit/blocks/');
for (const texture of files) {
    ...
    if (mcData.blocksByName[item]) {
        ...
        const properties = [
           "type=item",
           `items=minecraft:${data.name}`,
           `texture=${texture}`
        ]

        fs.writeFileSync(`static/resourcepacks/Vanilla/assets/minecraft/mcpatcher/cit/blocks/${item}.properties`, properties.join("\n"))
