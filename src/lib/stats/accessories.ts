...
import { getMissingAccessories } from './missing';

export async function getAccessories(
	userProfile: Member,
	armor: ProcessedItem[],
	accessoryBag: ProcessedItem[],
	inventory: ProcessedItem[],
	enderchest: ProcessedItem[],
	storage: ProcessedItem[]
) {
	const output = {
	...
	for (const accessory of accessoryBag.concat(
		inventory.filter((a) => a.categories && a.categories.includes('accessory'))
	)) {
		const id = helper.getId(accessory);
		if (id === '') {
			continue;
		...
		if (id in ACCESSORY_ALIASES || Object.keys(ACCESSORY_ALIASES).find((a) => ACCESSORY_ALIASES[a].includes(id))) {
			...
			for (const duplicate of accessoryDuplicates) {
				accessoryBag
					.concat(inventory.filter((a: ProcessedItem) => a.categories && a.categories.includes('accessory')))
					.map((a) => {
	...
	for (const item of enderchest.concat(storage)) {
		...
		if (
			item.categories &&
			!item.categories.includes('accessory') &&
