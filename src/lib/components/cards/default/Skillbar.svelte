...
<div
  class={cn(
    "group relative isolate flex grow basis-full flex-col sm:basis-1/3 sm:last:odd:grow sm:last:odd:basis-1/2",
  ...
  <Progress.Root
    ...
    class="relative ml-2 flex h-4 overflow-hidden rounded-full border bg-foreground/30"
    data-api={apiEnabled}>
    {#if apiEnabled && showXP}
      <div class="absolute inset-0 z-20 flex h-full justify-center">
    ...
    {/if}
    <div
      class="relative z-10 h-full flex-1 rounded-full data-[api=false]:hidden data-[api=true]:data-[maxed=false]:bg-primary data-[api=true]:data-[maxed=true]:bg-accent-3"
  ...
  <div
    class="absolute bottom-0 left-0 z-10 flex size-9 items-center justify-center rounded-full border p-1 drop-shadow-sm data-[api=false]:bg-gray-600 data-[maxed=false]:bg-primary data-[maxed=true]:bg-accent-3"
    data-maxed={isMaxed}
    data-api={apiEnabled}>
    <img
      loading="lazy"
      class="pointer-events-none size-6.5 [image-rendering:pixelated] data-[api=false]:grayscale"
