...
<div
  class="flex flex-col gap-2 *:motion-preset-focus *:motion-preset-slide-right *:motion-delay-[calc(sibling-index()*0.1s)] @md:flex-row @md:flex-wrap">
  {#key profile}
    {#if profile != null}
      {#if profile.joined != null}
        <AdditionStat
          text="Joined"
          data={formatDistanceToNowStrict(profile.joined, {
            addSuffix: true,
      ...
      {#if profile.bank != null && profile.personalBank != null}
        <AdditionStat
          text="Bank Account"
          data={`${formatNumber(profile.bank + profile.personalBank)} Coins`}
          asterisk={profile.bank && profile.personalBank ? true : false}>
          <div class="space-y-2">
            <Label class="gap-1">
              Bank:
      ...
      {#if profile.skills?.averageSkillLevel}
        <AdditionStat text="Average Skill Level" data={profile.skills.averageSkillLevel.toFixed(2)} asterisk={true}>
          <div class="max-w-xs space-y-2">
            ...
            <p class="max-w-xs font-medium text-muted-foreground">
    ...
    <svelte:boundary>
      {const networth = $derived(
        profileUUID != null && profileId != null ? await getProfileNetworth({ uuid: profileUUID, profileId }) : null
      )}
      {#snippet pending()}
      ...
      {#snippet failed(err, retry)}
        <div class="my-0 flex items-center gap-0.5 font-bold text-muted-foreground">
          <Popover.Root bind:open={networthOpen}>
            ...
            {#if isHttpError(err)}
              <Popover.Content
                class="glass bg-transparent glass-bg-popover"
                sideOffset={8}
                side="bottom"
                align="start"
                collisionPadding={6}
          ...
          <Button
            variant="link"
            onclick={retry}
            class="px-0 text-base font-bold text-foreground hover:text-foreground/80">Retry</Button>
