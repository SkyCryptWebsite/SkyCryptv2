import * as constants from '$lib/constants/skills';
 ...
 */
export function getSkillLevelCaps(profileData: Member, hypixelPlayer: Player) {
	return {
		farming: 50 + (profileData.jacobs_contest?.perks?.farming_level_cap || 0),
		taming: Math.min(Math.max(hypixelPlayer?.achievements?.skyblock_domesticator || 50, 50), 60),
		runecrafting: hypixelPlayer.newPackageRank ? 25 : 3
	};
}
