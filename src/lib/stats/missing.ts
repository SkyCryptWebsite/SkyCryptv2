...
function hasAccessory(accessories: Accessory[], accessory: Accessory | string, options = { ignoreRarity: false }) {
	...
	if (options.ignoreRarity === false) {
		...
		return accessories.some(
			(a) => a.id === id && constants.RARITIES.indexOf(a.rarity) >= constants.RARITIES.indexOf(rarity)
		);
	} else {
...
function getEnrichments(accessories: ProcessedItem[]) {
	...
	for (const item of accessories) {
		...
		if (
			...
			specialAccessory?.allowsEnrichment === false ||
...
function getMissing(items: ProcessedItem[], accessories: Accessory[]) {
	...
	for (const { id, rarity } of missing) {
		...
		if (
			(constants.getUpgradeList(id) && constants.getUpgradeList(id)[0] !== id) ||
			(ACCESSORY as SpecialAccessory).rarities
		) {
...
export async function getMissingAccessories(
	items: Accessories,
	userProfile: Member,
	packs: string[]
): Promise<AccessoriesOutput> {
