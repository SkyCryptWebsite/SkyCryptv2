...
{#snippet petCard(pet: ModelsStrippedPet)}
  <div>
    <Item piece={pet} />
    <p class="mt-2 text-center font-semibold data-[maxed=true]:text-accent-2" data-maxed={pet.level === pet.maxLevel}>
      LVL {pet.level}
    </p>
  </div>
{/snippet}
      ...
      <div class="contents space-y-4">
        <div class="rounded-xl border p-4">
          ...
          {#if pets.petScore != null}
            <AdditionStat
              text="Pet Score"
              data={`${pets.petScore.amount} (+${pets.petScore.stats?.magic_find} MF) `}
              asterisk={true}>
              <div class="max-w-xs space-y-4">
                ...
                <div class="flex flex-col">
                  {#each pets.petScore.reward?.toReversed() as { score, bonus, unlocked }, index (index)}
                    <div
                      class="group group text-foreground/80 data-[unlocked=true]:font-bold data-[unlocked=true]:text-foreground"
                      data-unlocked={unlocked}>
                      {score} Score:
                      <span class="text-minecraft-b/80 group-data-[unlocked=true]:text-minecraft-b"
                        >+{bonus} Magic Find</span>
                      {#if unlocked}
          ...
          <div class="rounded-xl border p-4">
            ...
            <div class="mt-4 flex items-center">
              ...
              <div class="ml-4 flex flex-col justify-center">
                <h4
                  class={cn(getRarityClass(activePet.rarity ?? "common", "text"), "text-xl font-bold capitalize")}
                  {@attach animateObfuscatedText}>
                  {(activePet.rarity ?? "common").toLowerCase()}
                  {@html renderLore((activePet.display_name ?? "").toLowerCase())}
                </h4>
