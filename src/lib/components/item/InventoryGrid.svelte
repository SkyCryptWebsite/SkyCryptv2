    ...
    <div class="grid grid-cols-[repeat(9,minmax(1.875rem,4.875rem))] place-content-center gap-1 @md:gap-1.5 @xl:gap-2">
      {#each items as item, index (index)}
        ...
        {#if item.texture_path}
          <div class="relative flex aspect-square items-center size-full overflow-clip border justify-center rounded-xl bg-foreground/5 data-[shine=true]:shine" data-shine={!preferences.performanceMode && shouldShine(item)}>
