...
class LonesomeMiner extends Node {
	...
	perk(level: number) {
		...
		return [
			`§7Increases §c${constants.SYMBOLS.strength} Strength, §9${constants.SYMBOLS.crit_chance} Crit Chance, §9${constants.SYMBOLS.crit_damage} Crit Damage, §a${constants.SYMBOLS.defense} Defense, and §c${constants.SYMBOLS.health} Health §7statistics gain by §a${val}% §7while in the Crystal Hollows.`
		];
	}
}
...
class ManiacMiner extends Node {
	...
	perk(level: number) {
		...
		return [
			...
			`§7Spends all your Mana and grants §a+${speed} §6${constants.SYMBOLS.mining_speed} Mining Speed §7for every 10 Mana spent, for §a${duration}s§7.`,
			`§8Cooldown: §a${cooldown}s`,
			'',
			'§8Pickaxe Abilities apply to all of your pickaxes. You can select a Pickaxe Ability from your Heart of the Mountain.',
			'',
...
class SkyMall extends Node {
	...
	perk(level: number) {
		return [
			...
			'',
			'§7Possible Buffs',
			`§8 ■ §7Gain §a+100 §6${constants.SYMBOLS.mining_speed} Mining Speed§7.`,
			`§8 ■ §7Gain §a+50 §6${constants.SYMBOLS.mining_fortune} Mining Fortune§7.`,
...
class MiningMadness extends Node {
	...
	perk(level: number) {
		return [
			`§7Grants §a+50 §6${constants.SYMBOLS.mining_speed} Mining Speed §7and §6${constants.SYMBOLS.mining_fortune} Mining Fortune§7.`
		];
	}
}
...
class FrontLoaded extends Node {
	...
	perk(level: number) {
		return [
			`§7Grants §a+100 §6${constants.SYMBOLS.mining_speed} Mining Speed §7and §6${constants.SYMBOLS.mining_fortune} Mining Fortune §7as well as §a+2 base powder §7for the first §e2,500 §7ores you mine in a day.`
		];
	}
}
...
class PrecisionMining extends Node {
	...
	perk(level: number) {
		return [
			`§7When mining ore, a particle target appears on the block that increases your §6${constants.SYMBOLS.mining_speed} Mining Speed §7by §a30% §7when aiming at it.`
		];
	}
}
...
class Crystallized extends Node {
	...
	perk(level: number) {
		...
		return [
			`§7Increases §6${speed} ${constants.SYMBOLS.mining_speed} Mining Speed §7and §6${fortune} ${constants.SYMBOLS.mining_fortune} Mining Fortune §7near §5Fallen Stars§7.`
		];
	}
}
...
class MiningSpeedBoost extends Node {
	...
	perk(level: number) {
		...
		return [
			...
			`§7Grants §a+${effect}% §6${constants.SYMBOLS.mining_speed} Mining Speed §7for §a${duration}s§7.`,
			`§8Cooldown: §a${cooldown}s`,
			'',
			'§8Pickaxe Abilities apply to all of your pickaxes. You can select a Pickaxe Ability from your Heart of the Mountain.',
			'',
...
class RagsOfRiches extends Node {
	...
	perk(level: number) {
		...
		return [
			`§7Grants §a+${val} §6${constants.SYMBOLS.mining_fortune} Mining Fortune §7while mining inside a §bGlaite Mineshaft`
		];
	}
}
...
class HazardousMiner extends Node {
	...
	perk() {
		...
		return [
			...
			`§7Grants §a+${boost} §6${constants.SYMBOLS.mining_speed} Mining Speed §7for §a${duration}s §7for each enemy within §a${radius} §7blocks`,
			`§8Cooldown: §a${cooldown}s`,
			'',
			'§8Pickaxe Abilities apply to all of your pickaxes. You can select a Pickaxe Ability from your Heart of the Mountain.',
			'',
...
class EagerAdventurer extends Node {
	...
	perk(level: number) {
		...
		return [
			`§7Grants §a+${val} §6${constants.SYMBOLS.mining_speed} Mining Speed §7when inside the §bGlacite Mineshafts§7.`
		];
	}
}
...
class MineshaftMayhem extends Node {
	...
	perk() {
		return [
			...
			`§8 ■ §7Gain §a100 §6${constants.SYMBOLS.mining_speed} Mining Speed§7.`,
			`§8 ■ §7Gain §a200 §6${constants.SYMBOLS.mining_fortune} Mining Fortune§7.`,
			`§8 ■ §7Gain §a+10 §b${constants.SYMBOLS.cold_resistence} Cold Resistence§7.`,
...
class HotmStats extends HotmItem {
	...
	get lore() {
		return [
			...
			'',
			`§9${constants.SYMBOLS.powder} Powder`,
...
export const GEMSTONE_CRYSTALS = [
	'jade',
	'amber',
	'amethyst',
	'sapphire',
