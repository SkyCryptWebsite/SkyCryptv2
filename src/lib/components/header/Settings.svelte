<script lang="ts">
  ...
  import { Avatar, Button, Label, Popover, Switch, Tabs } from "bits-ui";
...
<Popover.Root bind:open={settingsOpen}>
  <Popover.Trigger class="group relative my-1.5 flex shrink items-center justify-center gap-1 rounded-full bg-background/20 px-2.5 text-sm font-semibold text-text transition-all duration-100 ">
  ...
  <Popover.Content transition={flyAndScale} transitionConfig={{ duration: 300, y: -8 }} side="bottom" sideOffset={8} align="center" collisionPadding={8} class="z-[9999] min-w-[32rem] rounded-lg bg-background-grey/95 px-8 py-4">
    <Tabs.Root value="packs">
      <Tabs.List class="mb-4 flex gap-4 rounded-lg bg-text/30 p-2 font-semibold text-text">
        ...
        <Tabs.Trigger value="password" class="flex shrink items-center justify-center gap-1 rounded-lg px-2.5 py-1 text-sm font-semibold data-[state=active]:bg-icon">
      ...
      <Tabs.Content value="themes">Change your password here.</Tabs.Content>
    </Tabs.Root>
  </Popover.Content>
</Popover.Root>
